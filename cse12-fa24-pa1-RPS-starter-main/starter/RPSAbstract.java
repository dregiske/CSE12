...
public abstract class RPSAbstract implements RPSInterface {
    ...
    public boolean isValidMove(String move) {
        // implement a method that checks if the move is in the possibleMoves array
    ...
    public void playRPS(String playerMove, String cpuMove) {
        determineWinner(playerMove, cpuMove); // Play a game of rock, paper, scissors. It should;

        if (determineWinner(playerMove, cpuMove) == TIE_OUTCOME){ // 1. display outcome
                System.out.println(TIE); // 2. print message
                numTies = 1+ numTies; // 3. record moves in respective places
        }
        else if(determineWinner(playerMove, cpuMove) == PLAYER_WIN_OUTCOME){
                System.out.println(PLAYER_WIN);
                numPlayerWins = 1+ numPlayerWins;
        }
        else{
                System.out.println(CPU_WIN);
                numCPUWins = 1 + numCPUWins;
        }
        cpuMoves[numGames] = cpuMove;
        playerMoves[numGames] = playerMove;
        numGames = 1 + numGames; // 4. increment number of games played
    }
